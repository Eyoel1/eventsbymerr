...
export default function SettingsPage() {
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setImportStatus('✅ Import successful! Refreshing page…');
  ...
  return (
    <div className="page-content px-4 flex flex-col gap-5 max-w-lg mx-auto">
      <div className="pt-2">
        ...
        <p className="text-slate-400 text-xs font-medium">Preferences & data sovereignty</p>
      </div>
      ...
      <Card variant="elevated">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400 font-medium">High contrast OLED optimized</p>
          </div>
          <button
            id="dark-mode-toggle"
            onClick={toggleTheme}
            className={`relative w-14 h-8 rounded-full transition-all cursor-pointer border border-slate-700 ${
              theme === 'dark' ? 'bg-indigo-600' : 'bg-slate-800'
            }`}
          >
            <motion.div
              layout
        ...
        <div className="flex flex-col gap-4">
          <div className="flex gap-3">
            <div className="flex flex-col gap-1.5 flex-1">
              <label className="text-xs font-black text-slate-400 uppercase tracking-wider">Bodyweight</label>
              <input
                id="settings-weight"
                type="number"
                step="0.5"
                ...
                className="h-12 px-4 rounded-2xl border border-slate-700 bg-slate-900 text-white font-bold text-base"
              />
            </div>
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-black text-slate-400 uppercase tracking-wider">Days / Week</label>
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-black text-slate-400 uppercase tracking-wider">Injury / Limitations</label>
            <textarea
              id="settings-injuries"
              value={injuries}
              onChange={(e) => setInjuries(e.target.value)}
              rows={2}
              className="px-4 py-3 rounded-2xl border border-slate-700 bg-slate-900 text-white text-xs font-medium resize-none"
      ...
      <Card variant="elevated">
        ...
        <div className="flex flex-col gap-3">
          <div className="bg-amber-500/15 border border-amber-500/30 rounded-2xl p-3.5 flex items-start gap-3">
            <ShieldCheck size={20} className="text-amber-400 shrink-0 mt-0.5" />
            <p className="text-xs text-amber-200 font-medium leading-relaxed">
              All data lives strictly on your device. Export a backup JSON regularly to safeguard your progress history.
            </p>
          </div>
          <Button
            variant="secondary"
          ...
          />
          {importStatus && (
            <p className="text-xs text-indigo-400 font-bold text-center">{importStatus}</p>
          )}
        </div>
      </Card>
      ...
      <Card variant="glass">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <p className="font-black text-white text-base">Muscle Coach</p>
            <p className="text-xs text-slate-400 font-medium">v1.0 Progressive Overload Engine · PWA Ready</p>
          </div>
        </div>
      </Card>
    </div>
